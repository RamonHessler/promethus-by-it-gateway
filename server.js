...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(req.url?.startsWith("/bybit/klines?")){const u=new URL(req.url,"http://local");const symbol=u.searchParams.get("symbol")||"BTCUSDT",interval=u.searchParams.get("interval")||"60",limit=u.searchParams.get("limit")||"500";if(!/^[A-Z0-9]{3,20}$/.test(symbol))return json(res,400,{error:"invalid_symbol"});const x=await bybit("/v5/market/kline?category=spot&symbol="+symbol+"&interval="+interval+"&limit="+limit);const rows=x.data?.result?.list??[];return json(res,x.status,{reachable:x.status===200&&x.data?.retCode===0,symbol,closes:rows.slice().reverse().map(c=>Number(c[4])).filter(Number.isFinite)});}
